...
namespace Ham
{
    ...
    using coeff_dtype = std::complex<double>;
#else
    ...
    // helper function for real/complex Hamiltonian coeff calculation
    inline void _complexMultiplyAccu(const std::complex<double>& a, const std::complex<psi_dtype>& b, psi_dtype &e_loc_real, psi_dtype &e_loc_imag) {
        e_loc_real += a.real() * b.real() - a.imag() * b.imag();
        e_loc_imag += a.real() * b.imag() + a.imag() * b.real();
    };

    inline void _complexMultiplyAccu(const double a, const std::complex<psi_dtype>& b, psi_dtype &e_loc_real, psi_dtype &e_loc_imag) {
        e_loc_real += a * b.real();
        e_loc_imag += a * b.imag();
    };

    // coeff_dype maybe double or complex<double>
    inline coeff_dtype _scalarxRealOrComplex(psi_dtype scalar, coeff_dtype real_or_complex_number) {
        return coeff_dtype(scalar) * real_or_complex_number;
    }
    ...
    class Hamiltonian
    {
        ...
        private:
            ...
            std::complex<double> cplx_pow_neg_1i(int cnt) {
                static const std::array<double, 4> lookup_real = {1.0, 0.0, -1.0, 0.0};
                static const std::array<double, 4> lookup_imag = {0.0, -1.0, 0.0, 1.0};
                return std::complex<double>(lookup_real[cnt % 4], lookup_imag[cnt % 4]);
            }
    };
