...
// static float64 *g_coeffs = NULL;
    ...
    using coeff_dtype_cuda = cuDoubleComplex;
#else
    using coeff_dtype_cuda = float64;
#endif
static coeff_dtype_cuda *g_coeffs = NULL;

static dtype *g_pauli_mat12 = NULL;
static dtype *g_pauli_mat23 = NULL;

// CUDA kernel to handle complex multiplication and accumulation
__device__ void _complexMultiplyAccu(const cuDoubleComplex a, const cuDoubleComplex b, double &e_loc_real, double &e_loc_imag) {
    cuDoubleComplex prod = cuCmul(a, b);
    e_loc_real += cuCreal(prod);
    e_loc_imag += cuCimag(prod);
}

__device__ void _complexMultiplyAccu(const double a, const cuDoubleComplex b, double &e_loc_real, double &e_loc_imag) {
    e_loc_real += a * cuCreal(b);
    e_loc_imag += a * cuCimag(b);
}

__device__ void _complexMultiplyAccu(const cuDoubleComplex a, const cuFloatComplex b, float &e_loc_real, float &e_loc_imag) {
    cuDoubleComplex prod = cuCmul(a, make_cuDoubleComplex(cuCrealf(b), cuCimagf(b)));
    e_loc_real += cuCreal(prod);
    e_loc_imag += cuCimag(prod);
}

__device__ void _complexMultiplyAccu(const double a, const cuFloatComplex b, float &e_loc_real, float &e_loc_imag) {
    e_loc_real += a * cuCrealf(b);
    e_loc_imag += a * cuCimagf(b);
}

// res += scalar * {real|complex}
template<typename T>
__device__ void _scalarxRealOrComplex(const double scalar, const T real_number, T &res) {
    res += scalar * real_number;
}

template<>
__device__ void _scalarxRealOrComplex<cuDoubleComplex>(const double scalar, const cuDoubleComplex complex_number, cuDoubleComplex &res) {
    res = cuCadd(make_cuDoubleComplex(scalar*cuCreal(complex_number), scalar*cuCimag(complex_number)), res);
}

__device__ cuFloatComplex _make_cuComplex(const float real, const float imag) {
    return make_cuFloatComplex(real, imag);
}

__device__ cuDoubleComplex _make_cuComplex(const double real, const double imag) {
    return make_cuDoubleComplex(real, imag);
}

// Hamiltonian coeff initial
template<typename T>
__device__ void init_coeff(T &coef) {
    coef = T(0);
}

template<>
__device__ void init_coeff<cuDoubleComplex>(cuDoubleComplex &coef) {
    coef = make_cuDoubleComplex(0.0, 0.0);
}
    ...
    // const dtype *pauli_mat23)
{
    ...
    printf("[libeloc] set_indices_ham_int_opt BitOpt in GPU with psi_dtype: %s Hamiltonian coeff_dtype: %s memory occupied: %.4fMB\n", PRINT_TYPE(psi_dtype), PRINT_TYPE2(coeff_dtype), real_size);
    fflush(stdout);
}
#else
void set_indices_ham_int_opt(
    ...
    const dtype *pauli_mat23)
{
    ...
    cudaCheckErrors("cudaMemcpy failure");
    float32 real_size = (size_g_idxs + size_g_coeffs + size_g_pauli_mat12 + size_g_pauli_mat23) / 1024.0 / 1024.0;
    printf("[libeloc] set_indices_ham_int_opt in GPU with psi_dtype: %s Hamiltonian coeff_dtype: %s memory occupied: %.4fMB\n", PRINT_TYPE(psi_dtype), PRINT_TYPE2(coeff_dtype), real_size);
    fflush(stdout);
}
#endif
    ...
    psi_dtype *res_eloc_batch)
{
    ...
    for (int ii = index; ii < batch_size_cur_rank; ii+=stride) {
        ...
        for (int sid = 0; sid < NK; sid++) {
            // coeff_dtype_cuda coef = 0.0;
            coeff_dtype_cuda coef; init_coeff(coef);
            ...
            for (int i = st; i < ed; i++) {
                ...
                // coef += _sgn * coeffs[i];
                _scalarxRealOrComplex(_sgn, coeffs[i], coef);
                i_base += N;
            }
            // filter value < eps
            // if (FABS(coef) < eps) {
            ...
            for (int _ist = 0; _ist < batch_size; _ist++) {
                if (ks[_ist] == id) {
                    ...
                    _complexMultiplyAccu(coef, _make_cuComplex(vs[_ist*2], vs[_ist*2+1]), e_loc_real, e_loc_imag);
                    break;
                }
            }
            #else
            ...
            while (_ist < _ied) {
                ...
                if (ks[_imd] == id) {
                    ...
                    _complexMultiplyAccu(coef, _make_cuComplex(vs[_imd*2], vs[_imd*2+1]), e_loc_real, e_loc_imag);
                    break;
                }
    ...
    psi_dtype *res_eloc_batch)
{
    ...
    for (int ii = index; ii < batch_size_cur_rank; ii+=stride) {
        ...
        for (int sid = 0; sid < NK; sid++) {
            ...
            _complexMultiplyAccu(coef, _make_cuComplex(psi_real, psi_imag), e_loc_real, e_loc_imag);
        }
    ...
    psi_dtype *res_eloc_batch)
{
    ...
    const int32 n_qubits = g_n_qubits;
    const int64 *idxs = g_idxs;
    const coeff_dtype_cuda *coeffs = g_coeffs;
    const dtype *pauli_mat12 = g_pauli_mat12;
    const dtype *pauli_mat23 = g_pauli_mat23;
