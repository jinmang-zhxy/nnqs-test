...
def absolute_pow2_value(x):
    ...
    """
    x_star = conj(x)
    return real(scalar_mult(x, x_star))
...
if __name__ == "__main__":
    x = np.array([2.8+1.5j, 2.8+1.2j, -10-1.5j])
    # y = [1, 1, 1]
    # y = np.array([1.+0.j, 1.+0.j, 1.+0.j])
    # y = np.array([1., 1., 1.])
    y = np.array([1, 1, 1])
    print('x', x)
    print('y', y)
    ...
    z = np.dot(y, x)
    print('z', z)
    # x, y = np_to_torch(x), np_to_torch(y)
    # z = inner_prod(x, y)
